% ========== PREAMBLE ==========
% ---  Pengaturan dokumen ---
% Ukuran kertas A4, font 12pt dan halaman dua sisi
\documentclass[12pt,a4paper,twoside]{book}
% Margin
\usepackage[top=30mm,left=30mm,right=20mm,bottom=25mm]{geometry}
% Spasi
\usepackage[singlespacing]{setspace}
% Character encoding
\usepackage[utf8]{inputenc}
% Readability
\usepackage{microtype}
% Font
\usepackage{fontspec}
\usepackage{lmodern}

% --- Konten ---
% Bahasa
\usepackage[indonesian]{babel}
% Gambar
\usepackage{graphicx}
% Warna
\usepackage{xcolor}
\definecolor{biru}{RGB}{0,103,172}
\usepackage{pagecolor}
% Objek
\usepackage{tikz}
% Persamaan
\usepackage{amsfonts,amssymb,amsmath}
% Sampul
\usepackage{booktabs}
\usepackage{adjustbox}
\usepackage[absolute,overlay]{textpos}
% List
\usepackage[shortlabels]{enumitem}
\setlist[enumerate]{itemsep=-5pt}
% Tabel
\usepackage{longtable}
\usepackage{multirow}
\usepackage{multicol}
% Caption
\usepackage{caption}
\usepackage{subcaption}
% lines of space
\usepackage{needspace}
% Formatting chapter & section
\usepackage{textcase}
\usepackage{titlesec}
\titleformat{\chapter}
{\normalfont\Large\bfseries\centering\MakeUppercase}
{\MakeUppercase{\chaptertitlename}\ \thechapter}{10pt}{\Large}

\titlespacing*{\chapter}{30mm}{-5pt}{5mm}
\titlespacing{\section}{0pt}{\baselineskip}{0.2\baselineskip}

\setlength{\parindent}{0.5cm} % set the indent for each paragraph
\setlength{\parskip}{5pt} % remove the additional vertical space between paragraphs

% Hyperref
\usepackage[pdfborder={0 0 0}]{hyperref}
% Spacing paragraph
\usepackage{parskip}
\titleformat{\section}[hang]{\normalfont\bfseries}{\hspace{0pt}\thesection}{5pt}{\hspace{5pt}\large}[\vspace{0.5ex}]

\titleformat{\subsection}[hang]{\normalfont\bfseries}{\hspace{0pt}\thesubsection}{5pt}{\hspace{5pt}\large}[\vspace{0.5ex}]


% Nomor Halaman
\usepackage{fancyhdr}
% Sebelum daftar isi
\fancypagestyle{pages1to11}{
	\fancyhf{} % clear header and footer
	\fancyfoot[C]{\thepage} % center page number
}

% Setelah daftar isi
\fancypagestyle{otherpages}{
	\fancyhf{} % clear header and footer
	\fancyfoot[LE,RO]{\thepage} % alternating left and right page number
}

%----
%\pagestyle{fancy}
%\fancyhf{}
%\fancyfoot[C]{\ifnum\value{page}<12 \thepage
%	\else\ifodd\value{page}\rlap{\hspace{.75\textwidth}\thepage}
%	\else\llap{\thepage\hspace{.75\textwidth}}\fi\fi}
%\fancyfoot[LE,RO]{\thepage}
% ----
\renewcommand{\headrulewidth}{0pt}
% Changing case of text
\usepackage{textcase}
% Bibliography 
\usepackage{natbib}
\setcitestyle{open={(}, close={)}}
% Dummy text
\usepackage{lipsum}

\pagestyle{pages1to11}
% ========== KONTEN ==========
\begin{document}
	\input{Cover/cover_depan.tex}
	\input{Cover/cover_dalam.tex}
	\input{Cover/cover_dalam_en.tex}
	\pagenumbering{roman}
	\input{Lainnya/pengesahan.tex}
	\input{Lainnya/pengesahan_en.tex}
	\input{Lainnya/orisinalitas.tex}
	\input{Lainnya/orisinalitas_en.tex}
	\input{Abstrak/abstrak.tex}
	\input{Abstrak/abstrak_en.tex}
	\input{Lainnya/katapengantar.tex}
	\input{Lainnya/daftarisi.tex}

\pagestyle{otherpages}
% === BAB ====	
	\clearpage
	\pagenumbering{arabic}
	\singlespacing
	
	\include{Contents/1-Pendahuluan.tex}
	\include{Contents/2-TinjauanPustaka.tex}
	\include{Contents/3-Metodologi.tex}
	\include{Contents/4-Analisa Data dan Pembahasan.tex}
	\include{Contents/5-Kesimpulan.tex}
	\include{Contents/6-DaftarPustaka.tex}
	
\end{document}